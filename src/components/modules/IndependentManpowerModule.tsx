...
import { IdCard, Save, MapPin, Phone, Mail, Calendar, User, Home, CreditCard, Facebook, Link, Users, List, Search, Trash2, Eye, RefreshCw, ArrowLeft } from "lucide-react"
...
export default function IndependentManpowerModule() {
  ...
  useEffect(() => {
    // Filter IM list based on search term
    if (searchTerm.trim() === '') {
      setFilteredImList(imList)
    } else {
      const filtered = imList.filter(im => 
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${statusClasses[status as keyof typeof statusClasses] || statusClasses.pending}`}>
          ...
          <div className="p-6 border-b border-gray-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-600">IM#{selectedIM.imNumber} • {getStatusBadge(selectedIM.status || 'pending')}</p>
              </div>
            </div>
          </div>
              ...
              <div className="bg-white/50 rounded-lg border p-6">
                ...
                <dl className="space-y-3">
                  ...
                  <div>
                    ...
                    <dd className="text-base text-gray-900">{getStatusBadge(selectedIM.status || 'pending')}</dd>
                  </div>
                </dl>
              </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder="Search by name, IM number, email, or contact..."
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    className="pl-10 pr-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-orange focus:border-transparent bg-white text-sm w-80"
                  />
                </div>
                <button
                  onClick={loadIMList}
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredImList.map((im) => (
                      <tr key={im.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          {getStatusBadge(im.status || 'pending')}
                        </td>
